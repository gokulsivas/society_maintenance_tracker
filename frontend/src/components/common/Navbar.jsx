...
export default function Navbar() {
  ...
  const navLinkClass = ({ isActive }) =>
    `inline-flex items-center px-3.5 py-2 text-xs font-sans font-semibold uppercase tracking-wider transition-colors ${
      isActive
        ? 'bg-[#302a25] text-[#f5f2ec] border-b-2 border-[#d8cdbc]'
        : 'text-[#d8cdbc] hover:text-[#ffffff] hover:bg-[#302a25]'
    }`;

  const mobileNavLinkClass = ({ isActive }) =>
    `block px-3.5 py-2.5 text-sm font-sans font-semibold transition-colors ${
      isActive
        ? 'bg-[#302a25] text-[#f5f2ec] border-l-4 border-[#d8cdbc]'
        : 'text-[#d8cdbc] hover:text-[#ffffff] hover:bg-[#302a25]'
    }`;
  ...
  return (
    <nav className="bg-[#24211e] border-b border-[rgba(245,242,236,0.16)] text-[#f5f2ec] shadow-sm sticky top-0 z-40">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="w-10 h-10 rounded-none bg-[#302a25] hover:bg-[#3f3025] text-[#FAF8F5] border border-[rgba(245,242,236,0.2)] hover:border-[#f5f2ec] font-sans font-semibold text-xs tracking-wider flex items-center justify-center shadow-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#d8cdbc]"
              ...
              >
                {user?.avatar_url ? (
                  ...
                  <span className="tracking-wider font-sans text-[#f5f2ec]">{getUserInitials(user?.name)}</span>
                )}
              </button>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[#24211e] border-t border-[rgba(245,242,236,0.16)] px-3 pt-3 pb-4 space-y-1 text-[#f5f2ec]">
          {!isAdmin ? (
            <>
